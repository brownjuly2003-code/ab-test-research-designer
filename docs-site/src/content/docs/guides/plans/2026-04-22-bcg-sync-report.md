...
- Partial: `4.1.3` есть общий `/api/v1/results`, но нет project-scoped webhook; `4.2.1` есть deps + `en.json` + isolated i18n test, но нет полной app-wide integration; `4.4.1` есть `/docs`/`/redoc` + token auth + global rate limits, но нет API-key management/per-key limits.
