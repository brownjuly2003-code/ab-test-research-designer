...
- Scientific oracle gate (plan step 9): optional pinned SciPy/statsmodels/lifelines environment (`app/backend/requirements-oracle.txt`) plus `scripts/run_statistical_oracle.py`, producing `.ci-artifacts/statistical-oracle.json` with dependency versions, method-specific tolerances, and 57 differential/metamorphic checks across Student/F tails, binary intervals, exact/count/categorical tests, robust/paired/omnibus/survival/Cox, and ratio delta method. CI now uploads the oracle artifact and runs the job on PR/main, manual, and weekly scheduled workflows; production requirements stay unchanged.
